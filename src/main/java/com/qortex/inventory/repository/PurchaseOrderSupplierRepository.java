...
@Repository
@Slf4j
public class PurchaseOrderSupplierRepository {
    ...
    @Autowired
    public PurchaseOrderSupplierRepository(EntityManager entityManager,
    ...
    public Page<PurchaseOrder> listBySupplierId(PurchaseOrderSearch purchaseOrderSearch, PurchaseOrderPage purchaseOrderPage,String supplier_id) {
        ...
        Join<PurchaseOrder, Supplier> supplierJoin = purchaseOrderRoot.join("supplierId", JoinType.INNER);
        Predicate predicate = getPredicatesForPurchaseOrders(purchaseOrderSearch, purchaseOrderRoot, supplierJoin,supplier_id);
        criteriaQuery.where(predicate);

        setOrder(purchaseOrderPage, criteriaQuery, purchaseOrderRoot);
        long count = getPurchaseOrderCount(purchaseOrderSearch, supplier_id);
    ...
    private Predicate getPredicatesForPurchaseOrders(PurchaseOrderSearch purchaseOrderSearch,
                                                     ...
                                                     Join<PurchaseOrder, Supplier> supplierJoin, String supplier_id) {
            log.info("fetch all purchase orders by supplier id predicate");
            ...
            predicates.add(criteriaBuilder.equal(purchaseOrderRoot.get("supplierId"), supplier_id));
    ...
    public long getPurchaseOrderCount(PurchaseOrderSearch purchaseOrderSearch, String supplier_id) {
        ...
        Predicate predicate = getPredicatesForPurchaseOrders(purchaseOrderSearch, countRoot, supplierJoin, supplier_id);
